...
$('#bpmnDiagramView').on('shown.bs.modal', function (e) {
    ...
    if( bpmnResourcePath == undefined){
        alertify.error('BPMN resource path is missing');
        return;
    }    
    $.ajax({
            ...
            error: function () {
                alertify.error('Error occurred while getting bpmn diagram');
            }
        });
});
...
$("#search-result-table").delegate('button', 'click', function(e) {
    ...
    $.ajax({
        ...
        error: function () {
            alertify.error('Error occurred while associating process to bpmn resource');
        }
    });
