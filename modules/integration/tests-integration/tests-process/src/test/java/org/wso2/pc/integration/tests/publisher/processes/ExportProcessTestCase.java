...
public class ExportProcessTestCase extends PCIntegrationBaseTest {
    ...
    public void init() throws Exception {
        ...
        resourcePath =
                FrameworkPathUtil.getSystemResourceLocation() + "artifacts" + File.separator + "json" + File.separator
                        + "process" + File.separator + TEST_CREATE_PROCESS_JSON_FILE;
    ...
    public void addProcess()
        ...
        ClientResponse response = genericRestClient
                .geneticRestRequestPost(publisherProcessAPIBaseUrl + "create_process", MediaType.APPLICATION_JSON,
        ...
        Assert.assertTrue(response.getStatusCode() == PCIntegrationConstants.RESPONSE_CODE_OK,
                "Expected " + PCIntegrationConstants.RESPONSE_CODE_OK + "OK, Received " + response.getStatusCode());
        Assert.assertTrue(responseObject.get("error").toString().equals("false"), "Error while creating the process");
    }
    ...
    public void uploadPDF() throws IOException {
        ...
        PostMethod httpMethod = ArtifactUploadUtil
                .uploadDocument(resourcePath1, ASSOCIATED_PDF_NAME, ASSOCIATED_PDF_SUMMARY,
                        PCIntegrationConstants.PDF_EXTENSION, "NA", "file", PROCESS_NAME, PROCESS_VERSION, cookieHeader,
                        url, PCIntegrationConstants.APPLICATION_PDF_TYPE);
        Assert.assertTrue(httpMethod.getStatusCode() == PCIntegrationConstants.RESPONSE_CODE_OK,
                "Wrong status code ,Expected " + PCIntegrationConstants.RESPONSE_CODE_OK + ",Received " + httpMethod
                        .getStatusCode());
    }
    ...
    public void uploadBPMN() throws IOException {
        queryMap.put("type", "process");
        String resourcePath1 = FrameworkPathUtil.getSystemResourceLocation() + "artifacts" +
                File.separator + "BPMN" + File.separator + TEST_BPMN_FILE;
        String url = publisherProcessAPIBaseUrl + "upload_bpmn";
        PostMethod httpMethod = ArtifactUploadUtil
                .uploadBPMN(resourcePath1, PROCESS_NAME, PROCESS_VERSION, "BPMN", cookieHeader, url);
        Assert.assertTrue(httpMethod.getStatusCode() == PCIntegrationConstants.RESPONSE_CODE_OK,
                "Wrong status code ,Expected " + PCIntegrationConstants.RESPONSE_CODE_OK + " ,Received " + httpMethod
                        .getStatusCode());
    }
    ...
    public void addFlowChart() throws IOException, JSONException {
        String flowchartBody = readFile(
                FrameworkPathUtil.getSystemResourceLocation() + "artifacts" + File.separator + "json" + File.separator
                        + "process" + File.separator + TEST_FLOW_CHART_FILE);
        ...
        ClientResponse response = genericRestClient
                .geneticRestRequestPost(publisherProcessAPIBaseUrl + "upload_flowchart", MediaType.APPLICATION_JSON,
    ...
    public void cleanUp() throws Exception {
        ...
        Assert.assertTrue(((response.getStatusCode() == HttpStatus.OK.getCode())),
                "Wrong status code ,Expected " + PCIntegrationConstants.RESPONSE_CODE_OK +
